...
**Gate first.** Trivial per the method's triviality gate: just do it, verify with the one obvious check, report in two sentences. No stages, no workers. Everything else runs the four stages below in order.
...
2. **Evidence fan-out.** Spawn the evidence gatherers as parallel workers in ONE message, never sequentially:
   ...
   - each worker returns distilled findings with citations, never raw file dumps.
   One batch plus one follow-up batch is the budget; a third needs a stated reason.
   > **Framework note:** If your framework has no parallel worker tool, perform these lookups sequentially but batch independent reads in one message. Note this limitation in your report.
...
1. Work the checklist in the **main thread** (use the task tracker abstraction for your framework; tick items as they complete). Deciding and editing stay in the main thread; only searching and verifying fan out.
...
3. Independent mechanical items (same change across many files, isolated file generation) may fan out to parallel workers, in one message, with worktree isolation if they could touch the same files.
...
5. Mid-item ignorance is a pause, not a guess: the moment an edit would carry a fact from memory (a signature, a key, a figure), stop that item, fan out one research worker per the method's recall gate, and resume when it returns.
...
The loop is model-agnostic. Evidence and attacker workers are cheap-model-friendly; keep the main thread (deciding, editing) on the strongest model available, and give attackers higher effort than gatherers when a choice exists.
