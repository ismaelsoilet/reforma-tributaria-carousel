...
2. **Establish what actually changed.** Use git diff and git status (or a directory diff against a pristine reference when there is no repo). The diff is ground truth; the report is not. Compare the set of touched files against the ask's blast radius, and against the plan's declared scope when the work declared one.
   > **Framework note:** If your framework has no git operations, note this limitation. You can still verify by re-reading files and re-running checks, but you cannot diff against a pristine reference.
